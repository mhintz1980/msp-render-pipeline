...
### 2026-09-24 batch-3h rim-fix candidates: the shipped rig fails the orbit at TWO azimuths (az42 **and** az318); `pool_soft` and Rim-off clear the orbit metric at all 30; the strong-form byte clause failed and an A/A control displaces it, so the metric clause - bit-exact at all four pairs - is the licensed basis for reading `pool_soft`, with the look verdict still Mark's

Chain (all in `output/`): frozen contract `batch3h-rim-fix-probe-contract.md` (pre-declared rules 1-4 for the first read plus the reported set and the two ungated boundary metrics; appended pre-dispatch audit records the rotated OFF dispatch order and the slow-frame question, and the pre-dispatch fix that kept all three runs on one build; the post-dispatch A/A addendum quoted below). Results record `batch3h-rim-fix-results.md`; measurements `batch3h-measurements.json`, method `batch3h-measure.py`; pixel-level identity `batch3h-pixel-identity.json`, method `batch3h-pixel-identity.py`; A/A control `batch3h-aa-control.json`, method `batch3h-aa-control.py`; owner-review images `batch3h-looksheet.png`, `batch3h-looksheet-crops.png`, `batch3h-profile-az{42,318,222,90}.png`; per-azimuth second log `batch3h-azcost.py`.

**Dispatches (already executed; no re-dispatch this task).** Four 30-azimuth orbits (42→390, 12° steps), `jobs/rl300_05_studio-floor.json`, v2 env sha `617e63a7...`, all on ONE worker build `render_worker.py` sha256 `22441c8cb658...` (checked in all four `request.json`): REF, app `ap-Qhj6E9bVYdr0uLaNlJ7C0x`, no override, 30 frames 647.165 s (wall 704.131); SOFT `lighting.rim_profile=pool_soft`, app `ap-CwecGCwkon2tRZPmCTR6AL`, 646.300 s (wall 698.443); OFF `lighting.rim_enabled=false`, app `ap-2Q8PVvIX8Z8D0ZeiFBT1dj`, 540.185 s (wall 624.285); and REF2, the A/A control (added after the frames existed - the FOURTH dispatch is a control, not a look), no override, same plain orbit order, app `ap-M4MH82wnwglTPBmXHQNWnL`, 564.364 s (wall 623.306). Preflight estimate USD 1.9612 each (7.8448 total) recorded in `request.json` before dispatch; measured seconds are the sum of `result-az*.json:seconds` — rate-derived cost at 0.00030992 USD/s: 0.20057 / 0.20030 / 0.16741 / 0.17491, **total 0.74319 - RATE-DERIVED, no invoice queried** (the 1.9612-per-run figure is the estimator's conservative pre-dispatch number, not the spend). Logs: `batch3h-<tag>-modal.log`, `batch3h-<tag>-preflight.log`. OFF ran the same 30 manifests in the contract's pre-declared rotated order (az90 first) and records `orbit: null`; REF/SOFT/REF2 ran the plain order (az42 first).

**GATE RESULT (shipped pre-lens metric, unchanged 1.8, recomputed from the saved frames).** REF **FAILED** two azimuths: `BACKGROUND_BAND_STEP: az42 1.9745 > 1.8 (pre-lens)` and `BACKGROUND_BAND_STEP: az318 1.9848 > 1.8 (pre-lens)`, with worst azimuth az318 at 1.9848175048828125 (row 553) and az42 at 1.9744644165039062 (row 552); the surrounding band is az330 1.453765869140625, az390 1.4332199096679688, az54 1.2382965087890625, az306 1.2236480712890625 (rows 530/530/588/589). SOFT and OFF **PASSED** with no problems over all 30 azimuths (SOFT worst az330 0.8840484619140625 row 3; OFF worst az90 0.968292236328125 row 1031). REF2, the A/A control, reproduces REF's two crossings exactly (az42 1.9744644165039062 row 552; az318 1.9848175048828125) and likewise reports sequence `status: failed` on that evidence. Fidelity gate and grain determinism pass in all four runs; 30/30 frames rendered, exit 0, no failures; no encode was run (the sequence reports carry no encode or unique-frame fields).

**New measured finding 1 - the defect is a mirrored pair, not one azimuth.** Every earlier round rendered az42 (and az222) only; the 12° orbit shows the shipped rig crosses the 1.8 line at az42 **and** az318, which is where the contract's offline derivation placed the pool's visible arc. Any fix claim now has to hold at both crossings, and the relocation trap is exercised at 12° resolution only - a narrower defect between grid samples is still not excluded.

**New measured finding 2 - the ~180-260 s frame follows dispatch POSITION 1, not az42 (every earlier round misattributed it).** REF 257.763 s and SOFT 254.755 s took it on their first frame (az42); the OFF run, dispatched with the rotated order, took 180.671 s on az90 and 12.972 s on az42. The cost model should price one warm-up frame per container, not a specific azimuth.

**Pre-declared reproduction checks - metric PASSES, byte clause FAILS, and the A/A control refutes the byte clause's premise.** REF az42/az222 vs 3g baseline and OFF az42/az222 vs 3g rim-off are bit-exact on `max_step`, step row and both half-frame values at all four pairs (REF az42 1.9744644165039062 row 552; OFF az42 0.7167434692382812 row 3; etc.). The strong form - byte-identical `composite-prelens.png` - FAILED at all four pairs: 409 / 525 / 511 / 471 pixels of 2,250,000 differ by exactly one channel level (max delta 1, none above 2, mattes byte-identical); the bytes do differ and that stands as measured. A fourth dispatch then ran as an A/A control - the SAME build, no override, same orbit order (`batch3h-aa-control.json`): REF vs REF2 differ by 527 px (az42) and 471 px (az222), max delta 1, `mask.png` byte-identical - a same-build spread of the same order as, and larger at az42 than, the cross-build 409 / 525. The contract's post-dispatch addendum states the rule verbatim: if two runs of ONE build differ by the same handful of +/-1-LSB pixels, the byte clause's premise (bit-determinism of this pipeline) is refuted by measurement and the metric clause is the licensed basis for reading SOFT. That is the branch the control lands on, so the record now reads SOFT on the metric clause (bit-exact at all four pairs plus the control) instead of withholding it. The record still makes no look verdict of its own and treats no threshold as a formality; whether +/-1-LSB on 0.02 % of pixels is anything more than engine sampling variance is adjudicated by the addendum, not here, and the byte difference is on file in `batch3h-pixel-identity.json`.

**Mechanism note (interpretation, not a threshold argument, and not a look claim).** Where the gate passes, the tonal difference is *spread*, not removed: at az42 the peak step leaves the pool edge (row 552 -> row 3), the per-column ramp at x=1600 falls from +3.9999923706054688 to +1.8860015869140625 and at x=1750 from +3.587005615234375 to +2, `smoothed_step` falls 1.596638149685333 -> 0.5177680121527573, while `excursion` is unchanged (86.90695190429688 -> 86.91739654541016) and the product proxies barely move (mean luma 133.24659729003906 -> 133.22203063964844 at az42). The contract predicted a ~4.1x wider transition from `spot_blend = 0.35` / `shadow_soft_size = 0.35r`; the measurement is consistent with that. A pass on the shipped metric is not "the ramp is fixed".

**Look, separate from gates.** `batch3h-looksheet.png` (three runs, native resolution) and the profile plots are the review artifacts. The look verdict is Mark's call; the metric clause now licenses reporting `pool_soft` as a candidate look (bit-exact at all four pairs, plus the A/A control above), which is a licence to look, not an acceptance - and a pass on the shipped metric is still not "the ramp is fixed".

**Owner sign-off pending, unchanged:** the az42 production gate result stands FAILED for REF (and now for az318 as well, and reproduced by the REF2 control); the v2 env az~259/el~28.4 steep edge, the LDR clipping/headroom reading, the half-azimuth-step target, any full-orbit claim, and owner look acceptance all remain unresolved. The A/A control settles only the reproduction question, on the metric clause (same-build +/-1-LSB spread of the same order as the cross-build spread, `batch3h-aa-control.json`); it changes no gate and is not an acceptance. No threshold widened (ruling 4); no `jobs/` manifest, rig, source `.blend`, or asset changed by this round, while the pre-dispatch B1/N1 correction changed `render_worker.py` / `scripts/cloud_job_render.py` and added six tests, all committed as `e835c7f`.

### 2026-09-24 T-V2 pre-flight: elevational-edge probe az250-270 @1° — 21/21 rendered, all sequence gates PASS, worst pre-lens step 0.8603 (az267) vs the unchanged 1.8 gate

Owner-approved probe (video brief §13.5; ruling 4 of 2026-09-24, no further gate). One dispatch, `scripts/cloud_job_render.py --manifest jobs/rl300_05_studio-floor.json --azimuths 250,251,...,270 --output-dir output/tv2-edgeprobe-az250-270-20260924 --execute`, no `--set` (the job of record carries `rim_profile: pool_soft`; non-executing preflight `output/tv2-edgeprobe-preflight-20260924` confirmed all 21 frame manifests carry exactly `"pool_soft"` and no other optional lighting key). Worker `render_worker.py` sha256 `22441c8cb658...` — the same build as all four 3h orbits. App `ap-h5KaHUYDNh4KOg87QiBE6Z`, L4, exit 0.

**Estimate before:** `request.json` `cost_estimate_usd` 1.3941 (the estimator's conservative 101.65 s/frame local basis × 2 passes); brief §13.5 rate-derived estimate USD 0.16. **Measured after:** 21 frames, sum of `result-az*.json:seconds` **512.249 s** (first frame 240.113 s = the position-1 warm-up the 3h round identified; warm median 13.477 s), wall 564.053 s; **rate-derived USD 0.15876 at 0.00030992 USD/s — RATE-DERIVED, no invoice queried.** The §13.5 estimate held to within 1 %.

**Gates** (`scripts/probe_sequence.py --run-dir output/tv2-edgeprobe-az250-270-20260924 --expect-frames 21 --no-encode`, exit 0, `sequence-report.json` status `passed`, problems `[]`): 21 frames as declared; product fidelity gate all pass; grain deterministic; no outlier frames; frame MAE median 3.676 / max 3.733; max |second difference| coverage 0.00057, mean luma 0.0253, p95 luma 1.0. **Background-band pre-lens step, per azimuth (gate 1.8, unchanged):** 250 0.786, 251 0.795, 252 0.831, 253 0.786, 254 0.781, 255 0.786, 256 0.827, 257 0.767, 258 0.782, 259 0.803, 260 0.833, 261 0.802, 262 0.833, 263 0.823, 264 0.852, 265 0.813, 266 0.848, **267 0.860 (max)**, 268 0.843, 269 0.844, 270 0.841 — headroom ≥ 0.94 against the gate everywhere; az259 (the env's measured el~28.4° edge azimuth) is 0.803, unremarkable against its neighbours.

**Cross-check against the 3h SOFT orbit at the two shared azimuths:** az258 0.7822 vs 0.7829, az270 0.8413 vs 0.8420 — agreement to 0.0007, consistent with the measured non-bit-determinism of this pipeline (same build, different dispatch), not bit-exact and not claimed to be.

**Reading.** At 1° resolution across the ~259 edge sector the accepted `pool_soft` rig shows no narrow defect the 12° orbit could have missed; at the job-of-record camera, the environment's measured edge produces no background-band step above 0.861 (interpretation of the measured metric; the edge's visibility elsewhere in the frame was not separately measured). This clears the §13.5 pre-flight for the full-orbit spend. It is NOT a full-orbit claim (owner ruling: open until a 360-frame run exists) and NOT a look acceptance. Evidence: `output/tv2-edgeprobe-az250-270-20260924/`, `output/tv2-edgeprobe-modal.log`, `output/tv2-edgeprobe-gate.log` (gitignored).

### 2026-09-24 T-V2 first full orbit: 360 frames @1° — 360/360 rendered, zero failed frames, every sequence gate PASSES; MP4 + WebM encoded and gated — artifact `awaiting_owner_review`

Dispatched after the §13.5 pre-flight passed (same session, same worker build `22441c8cb658...`, v2 env, job of record `jobs/rl300_05_studio-floor.json` carrying the accepted `pool_soft`, no `--set`). Command: `scripts/cloud_job_render.py --manifest jobs/rl300_05_studio-floor.json --orbit 360 --output-dir output/tv2-orbit360-20260924 --resume --execute` (first use of the new T-V2a `--resume`; one invocation, so `resume-log.json` holds one entry with 360 pending and `status.json` reads `resumed: false` — no crash occurred, nothing was re-paid). Non-executing preflight first: 360 unique frames az42→az401 at `step_deg 1.0`, all manifests `rim_profile: "pool_soft"`, no other optional lighting key. App `ap-d0yPBQPKaOrJo7Y1P07xxW`, L4, exit 0.

**Estimate before:** `request.json` `cost_estimate_usd` 22.7534 (the estimator's conservative local 101.65 s/frame × 2 passes basis — ~14× the real rate; recorded, not relied on) and the architect's rate-derived estimate ~USD 1.58 (one ~240 s position-1 frame + 359 × ~13.5 s). **Measured after:** sum of `result-az*.json:seconds` **5337.800 s** (position-1 frame 265.836 s, warm median 13.667 s), wall 6026.983 s (~1 h 40 m); **rate-derived USD 1.65429 at 0.00030992 USD/s — RATE-DERIVED, no invoice queried.** Within 5 % of the rate-derived estimate.

**Gates** (`scripts/probe_sequence.py --run-dir output/tv2-orbit360-20260924 --expect-frames 360 --framerate 30`, exit 0, status `passed`, problems `[]`): 360 frames as declared; product fidelity gate all pass; grain deterministic; no neighbour outlier frames (factor 3.0); frame MAE median 4.594 / max 8.390; max |second difference| coverage 0.0033, mean luma 1.030, p95 luma 7.0; **background-band pre-lens step: worst 0.9043 at az361 against the unchanged 1.8 gate, zero azimuths above 1.5** (az42/az318, the shipped rig's two 3h crossings, now clear at 1° resolution with `pool_soft`). Encodes (one pass each, no loop repeat): `turntable-loop.mp4` H.264 crf 18, 360 encoded frames, decode-back PSNR 38.69 / 38.20 / 37.69 dB; `turntable-loop.webm` VP9 crf 24, 360 encoded frames, decode-back PSNR 40.98 / 38.59 / 37.85 dB — both above the unchanged 35.0 dB floor. Files 24.7 MB / 7.7 MB, 12 s at 30 fps.

**What this closes and what it does not.** This is the 360-frame run the 2026-09-24 ruling 5 made the precondition of any full-orbit claim; it now exists and every declared gate passes on it. Per standing ruling 3, a passing gate is NOT acceptance: the loop is `awaiting_owner_review` and the full-orbit/video acceptance is Mark's call on watching it. Evidence (gitignored): `output/tv2-orbit360-20260924/{turntable-loop.mp4,turntable-loop.webm,sequence-report.json,status.json,resume-log.json}`, `output/tv2-orbit360-modal.log`, `output/tv2-orbit360-gate.log`, contact sheet `output/tv2-orbit360-sheet.png`.

**Month-to-date rate-derived spend on this branch's recorded Modal frames** (sum of every `output/**/result-*.json` seconds × rate, frame seconds only): ~USD 3.7 against the USD 30 ceiling.

**Addendum, same date — orbit brightness swing measured from the stored frames (no new dispatch), and acceptance.** Sampling every 10th azimuth of the 360 orbit over product pixels (matte alpha > 250) on `composite-prelens.png`: mean luma **63.38 at az182 (minimum)** to **120.28 at az42 (maximum)**, a swing of **1.9×**; median luma 76.33 → 146.00, also 1.9×. This is the same order as §6.2's 2.0× mean swing and far from its 3.6× median swing on the single-plate `studio-dark` job, and nothing like its 36/255 median collapse (this orbit's worst median is 76.33). The swing is a smooth gradient — the sequence gates found **zero neighbour outliers** and a worst mean-luma second difference of 1.03/255, so it reads as the machine turning away from the key, not as pumping; the §9 criterion-1 concern was pumping, and the orbit's own whole-sequence statistics address it. Product coverage moves 0.245 → 0.455 purely with the short-end/long-side orientation of the machine, so it is geometry, not lighting. **Mark accepted this orbit verbatim the same date ("i approve") — recorded in `docs/rl300-parity.md`; the run is no longer `awaiting_owner_review`.**
